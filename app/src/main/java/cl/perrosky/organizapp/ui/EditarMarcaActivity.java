...
public class EditarMarcaActivity extends AppCompatActivity {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        menu.findItem(R.id.guardar).setVisible(true).setTitle(R.string.btn_save_marca);

        if(marcaEnEdicion()){
            menu.findItem(R.id.eliminar).setVisible(true).setTitle(R.string.btn_remove_marca);
        }
        return true;
    }
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()){
            ...
            case R.id.guardar:
                if (validarMarca()){
                    (new MarcaDataSource(this)).guardarMarca(marca);
    ...
    private boolean marcaEnEdicion(){
        return (marca!=null && marca.getId() > 0);
    }

    private boolean eliminarMarca(){
        int retorno = (new MarcaDataSource(this)).eliminarMarca(marca);
        return (retorno == 1);
    }
