...
public class DbOpenHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(Modelo.CATEGORIA.getInsert( "4, 'Antiacidos', 'provoca la disminución de las secreciones gástricas. Si disminuye la acidez, se previene la aparición de úlceras. Un ejemplo conocido es el Omeprazol.\n\nEn estos fármacos, los efectos secundarios más importantes son las alteraciones del tránsito intestinal (diarrea o estreñimiento).'"));
        db.execSQL(Modelo.CATEGORIA.getInsert( "5, 'Antialérgicos', 'En esta categoría se agrupan fármacos que tienen la finalidad de combatir los efectos negativos de las reacciones alérgicas o la hipersensibilidad.\n\nLos más populares son los fármacos de la familia de antihistamínicos, cuyo mecanismo de acción influye sobre la histamina, la cual tiene un importante papel en la alergias. Sus reacciones adversas son mínimas, pero puede ocasionar diarrea, somnolencia, fatiga o cefaleas.'"));
        db.execSQL(Modelo.CATEGORIA.getInsert( "6, 'Antidiarreicos', 'Los antidiarreicos son tipos de medicamentos que tienen como propósito aliviar y detener los efectos de la diarrea. Los más utilizados actualmente son fármacos que inhiben la motilidad del intestino, lo que favorece la retención para conseguir una mayor consistencia y volumen en las heces. Las reacciones adversas de estos fármacos son mínimas, aunque se han registrado algunas como el dolor abdominal o el estreñimiento.'"));
        db.execSQL(Modelo.CATEGORIA.getInsert( "7, 'Laxantes', 'Los laxantes son recetados para el caso contrario, en otras palabras, para resolver problemas de estreñimiento por un aumento del movimiento intestinal o por lubricación. Su uso debe ser moderado y como apoyo, porque un tratamiento prolongado hace que el intestino no trabaje correctamente, disminuyendo su capacidad de absorción de nutrientes.'"));
        db.execSQL(Modelo.CATEGORIA.getInsert( "8, 'Antiinfecciosos', 'Este tipo de medicamentos están recetados para hacer frente a infecciones. Dependiendo del agente infeccioso, se clasifican en antibióticos (contra bacterias), antifúngicos (contra hongos), antivirales (contra virus) y antiparasitarios (contra parásitos).'"));
        db.execSQL(Modelo.CATEGORIA.getInsert( "9, 'Antiinflamatorios', 'Como el propio nombre indica, son fármacos que tienen como finalidad reducir los efecto de la inflamación. Los más recetados son los catalogados como AINES, que además de disminuir la inflamación, tienen efectos analgésicos.'"));
        db.execSQL(Modelo.CATEGORIA.getInsert( "10, 'Antipiréticos', 'Los fármacos antipiréticos son un tipo de medicamentos que tienen la capacidad de reducir la fiebre. Entre los más conocidos están la aspirina, el ibuprofeno y el paracetamol, que también presentan otras funciones.'"));
        db.execSQL(Modelo.CATEGORIA.getInsert( "11, 'Antitusivos', 'Son fármacos que se recetan para tratar de reducir la tos no productiva, es decir, que no libera mucosidad. Se debe tener sumo cuidado con su dosis, ya que algunos de ellos, como la codeína, producen adicción.'"));
        db.execSQL(Modelo.CATEGORIA.getInsert( "12, 'Mucoliticos', 'son medicamentos que se recomiendan cuando la mucosidad dificulta una respiración correcta. Sus efectos secundarios son menores, como cefaleas o reacciones alérgicas.'"));
